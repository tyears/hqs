...
<div class="page-container">
	<form action="area/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		<input type="hidden" name="fkId" value="${params.fkId}"/>
		<input type="hidden" name="level" value="${params.level}"/>
		区域名称：<input type="text" id="areaName" name="areaName" value="${params.areaName}" placeholder="区域名称" style="width:250px" class="input-text"/>
		<button class="btn btn-success" onclick="query();" type="button"><i class="Hui-iconfont">&#xe665;</i> 搜列表</button>
	</div>
	</form>
	<form enctype="multipart/form-data" id="myForm" action="area/excelProduct.htm" method="post">
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a class="btn btn-success radius" href="javascript:window.open('template/area.xlsx');">
...
function showItems(id){
    ...
    var index = parent.layer.open({
        ...
        title: title,
        area: ['1000px', '500px'],
        content: "<%=basePath%>areaProduct/toQuery.htm?areaId="+id
    });
    //layer.full(index);
}
...
function edit(id){
	...
	var index = parent.layer.open({
		...
		content: "<%=basePath%>area/toEdit.htm?id="+id+"&fkId=${params.fkId}&level=${params.level}"
	});
	//layer.full(index);
}
...
function deleteData(id,isParent){
    ...
    var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
        $.ajax({
            type:"post",
            url:"<%=basePath%>area/deleteData.htm",
            dataType:"json",
            async:false,
...
$(function () {
    $('#path').change(function(){
        if(checkData()){
            layer.load(1);
            $("#myForm").ajaxSubmit({
                url:"<%=basePath%>area/excelProduct.htm",
                dataType:"json",
                success:function(data){
                    if(data.result){
                        layer.msg(data.msg, {
