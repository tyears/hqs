...
<div class="page-container">
	<form action="product/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
	...
	<form enctype="multipart/form-data" id="myForm" action="product/excelProduct.htm" method="post">
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a class="btn btn-success radius" href="javascript:window.open('template/product.xlsx');">
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>product/toEdit.htm?id="+id
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>product/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
...
$(function () {
    $('#path').change(function(){
        if(checkData()){
            layer.load(1);
            $("#myForm").ajaxSubmit({
                url:"<%=basePath%>product/excelProduct.htm",
                dataType:"json",
                success:function(data){
                    if(data.result){
                        layer.msg(data.msg, {
