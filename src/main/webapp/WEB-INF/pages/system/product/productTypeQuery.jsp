...
<div class="page-container">
	<form action="productType/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		类别名称：<input type="text" id="name" name="name" value="${params.name}" placeholder="类别名称" style="width:250px" class="input-text"/>
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>productType/toEdit.htm?id="+id
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>productType/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
