...
<body>
	...
	<form action="role/saveRoleRights.htm" id="myForm" method="post" >
...
$(document).ready(function(){
	...
	$.ajax({
		type:"post",
		url:"<%=basePath%>role/getRightsNodes.htm",
		dataType:"json",
		async:true,
		data:{roleId:'${roleId}'},
		success:function(data){
			...
			treeNodes = data;
			//初始化树
			zTree = $.fn.zTree.init($("#rightsTree"), setting ,treeNodes);
		},
		error:function(){
			layer.close(loadIndex);
...
function save(){
	...
	for(var i=0;i<checkedModule.length;i++){
		var inputStr = "<input type='hidden' name='roleRightsList["+i+"].roleId' value='"+roleId+"'/>";
		inputStr += "<input type='hidden' name='roleRightsList["+i+"].rightsId' value='"+checkedModule[i].id+"'/>";
		myForm.append(inputStr);
	}
	$.ajax({
		type:"post",
		url:"<%=basePath%>role/saveRoleRights.htm",
		dataType:"json",
		async:true,
