...
<div class="page-container">
	<form action="role/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>role/toEdit.htm?id="+id
	});
	//layer.full(index);
}
...
function powerGive(roleId){
	var index = layer.open({
		...
		content: "<%=basePath%>role/toRightsTreeUi.htm?roleId="+roleId
	});
}
/*角色单个-删除*/
function deleteData(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>role/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
