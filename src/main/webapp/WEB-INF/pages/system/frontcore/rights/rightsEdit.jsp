...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" autocomplete="off" placeholder="权限名称" id="name" name="name" value="${info.name}"/>
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>权限编码：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" autocomplete="off" placeholder="权限编码" id="code" name="code" value="${info.code}"/>
		</div>
	</div>
	<div class="row cl">
		...
		$("#myForm").validate({
			rules:{
				...
				"code":{
					required:true,
                    remote:{
                        url:"<%=basePath%>rights/checkCode.htm?id=${info.id}",
                        type:"post",
                        dataType: "json",
                        data:{//要传递的数据
                            code:function(){
			...
			submitHandler:function(form){
				...
				$.ajax({
					type:"post",
					url:"<%=basePath%>rights/editSave.htm",
					dataType:"json",
					async:false,
					data:$(form).serialize(),
