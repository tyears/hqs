...
<div class="page-container">
	<form action="user/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		用户名：<input type="text" name="userName" id="userName" value="${params.userName}" placeholder="用户名" style="width:150px" class="input-text"/>
		姓名：<input type="text" name="name" id="name" value="${params.name}" placeholder="姓名" style="width:150px" class="input-text"/>
		<span class="select-box" style="width:150px">
			<select class="select" name="departId" id="departId" size="1">
		...
		<span class="select-box" style="width:150px">
			<select class="select" id="userType" name="userType">
				...
				<option value="1" ${params.userType=='1'?'selected':''}>经理</option>
				<option value="0" ${params.userType=='0'?'selected':''}>员工</option>
			</select>
		</span>
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>user/toEdit.htm?id="+id
	});
}
...
function deleteData(id){
	var layerIndex = layer.confirm('此操作影响巨大，确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>user/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
...
function updateState(id,state){
	...
	var layerIndex = layer.confirm(tip,{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>user/changeStateById.htm",
			dataType:"json",
			async:true,
			data:{"id":id},
			success:function(data){
...
function roleGive(userId){
	var index = layer.open({
		...
		content: "<%=basePath%>user/toSetRole.htm?userId="+userId
	});
}
</script>
