...
<div class="page-container">
	<form action="article/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		所属类别：<select id="articleTypeId" name="articleTypeId" style="width:150px" class="input-text">
		<option value="" >--请选择--</option>
			<c:forEach items="${articleTypeList}" var="articleType">
				<option value="${articleType.id}" ${articleType.id==params.articleTypeId?'selected':''}>${articleType.typeName}</option>
			</c:forEach>
		</select>
		标题：<input type="text" id="title" name="title" value="${params.title}" placeholder="标题" style="width:150px" class="input-text"/>
		<button class="btn btn-success" onclick="query();" type="button"><i class="Hui-iconfont">&#xe665;</i> 搜列表</button>
	</div>
	</form>
	<form enctype="multipart/form-data" id="myForm" action="article/excelArticle.htm" method="post">
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			<a class="btn btn-primary radius" onclick="edit();">
				<i class="Hui-iconfont">&#xe600;</i>添加
			</a>
			<a class=" btn btn-danger radius" onclick="dels();">
			...
			<a class="btn btn-success radius" href="javascript:window.open('template/article.xlsx');">
...
//		area:['800px','600px'],
		...
		content: "<%=basePath%>article/toEdit.htm?id="+id
	});
	layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>article/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
...
function dels() {
	...
	if(!ids){
        layer.msg("请选择要删除的文章",{icon:2,time:1500})
        return;
	}
	alert(ids)
	$.post("article/deleteIds.htm",{ids:ids},function (data) {
		if(data.result){
...
$(function () {
    $('#path').change(function(){
        if(checkData()){
            layer.load(1);
            $("#myForm").ajaxSubmit({
                url:"<%=basePath%>article/excelArticle.htm",
                dataType:"json",
                success:function(data){
                    if(data.result){
                        layer.msg(data.msg, {
