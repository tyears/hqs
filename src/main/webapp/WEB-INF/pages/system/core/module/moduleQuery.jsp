...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i>资源菜单管理<span class="c-gray en">&gt;</span>资源菜单列表 <a id="refash" class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<form action="sysModule/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		<input type="hidden" name="fkId" value="${params.fkId}"/>
		<input type="hidden" name="level" value="${params.level}"/>
		资源名称：<input type="text" id="name" name="name" value="${params.name}" placeholder="资源名称" style="width:250px" class="input-text"/>
		<button class="btn btn-success" onclick="query();" type="button"><i class="Hui-iconfont">&#xe665;</i> 搜资源</button>
	</div>
	</form>
...
function edit(id){
	...
	var index = parent.layer.open({
		...
		content: "<%=basePath%>sysModule/toEdit.htm?id="+id+"&fkId=${params.fkId}&level=${params.level}"
	});
	//layer.full(index);
}
...
function deleteData(id,isParent){
	if(isParent=="true"){
		layer.msg("请先删除下级资源",{time:1000,icon:2});
		return;
	}
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>sysModule/deleteData.htm",
			dataType:"json",
			async:false,
