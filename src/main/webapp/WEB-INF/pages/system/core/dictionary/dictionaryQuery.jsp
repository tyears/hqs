...
<div class="page-container">
	<form action="sysDictionary/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		...
		<button class="btn btn-success" onclick="query();" type="button"><i class="Hui-iconfont">&#xe665;</i> 搜字典</button>
	</div>
	</form>
...
function showItems(id){
	var title = "<i class='Hui-iconfont'>&#xe627;</i>字典项列表";
	var index = layer.open({
		type: 2,
		title: title,
		area: ['700px', '450px'],
		content: "<%=basePath%>sysDictionaryMenu/toQuery.htm?fkId="+id
	});
	//layer.full(index);
}
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>sysDictionary/toEdit.htm?id="+id
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>sysDictionary/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
