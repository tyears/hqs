...
<div class="page-container">
	<form action="sysDictionaryMenu/listData.htm" method="post" id="queryForm">
	<div class="text-c">
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>sysDictionaryMenu/toEdit.htm?id="+id+"&fkId=${params.fkId}"
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>sysDictionaryMenu/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
