...
<article class="page-container">
	...
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>汉语表名：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" autocomplete="off" placeholder="汉语表名" id="name" name="name" value="${info.name}"/>
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>表名：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" autocomplete="off" placeholder="表名" id="tableName" name="tableName" value="${info.tableName}"/>
		</div>
	</div>
	</form>
</article>
	...
	$(function(){
		...
		$("#myForm").validate({
			...
			submitHandler:function(form){
				...
				$.ajax({
					type:"post",
					url:"<%=basePath%>generatorTable/editSave.htm",
					dataType:"json",
					async:false,
					data:$(form).serialize(),
