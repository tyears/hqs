...
<div class="page-container">
	<form action="dealerProduct/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		<input type="hidden" id="dealerId" name="dealerId" value="${params.dealerId}"/>
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>dealerProduct/toEdit.htm?id="+id+"&dealerId=${params.dealerId}"
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>dealerProduct/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
