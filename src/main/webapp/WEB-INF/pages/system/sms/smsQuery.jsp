...
<div class="page-container">
	<form action="sms/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		操作人：<input type="text" id="userName" name="userName" value="${params.userName}" placeholder="操作人" style="width:250px" class="input-text"/>
		接收号码：<input type="text" id="tel" name="tel" value="${params.tel}" placeholder="接收号码" style="width:250px" class="input-text"/>
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>sms/toEdit.htm?id="+id
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>sms/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
