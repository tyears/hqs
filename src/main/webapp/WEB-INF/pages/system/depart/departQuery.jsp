...
<div class="page-container">
	<form action="depart/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		部门名称：<input type="text" id="departName" name="departName" value="${params.departName}" placeholder="部门名称" style="width:250px" class="input-text"/>
		<button class="btn btn-success" onclick="query();" type="button"><i class="Hui-iconfont">&#xe665;</i> 搜部门</button>
	</div>
	</form>
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>depart/toEdit.htm?id="+id
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>depart/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
