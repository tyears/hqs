...
<div class="page-container">
	<form action="food/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		<input type="hidden" id="pageNum" name="pageNum" value="1"/>
		<input type="hidden" id="pageSize" name="pageSize" value="10"/>
		食品名称：<input type="text" id="foodName" name="foodName" value="${params.foodName}" placeholder="食品名称" style="width:250px" class="input-text"/>
		食品简拼：<input type="text" id="jianpin" name="jianpin" value="${params.jianpin}" placeholder="食品简拼" style="width:250px" class="input-text"/>
		<span class="select-box" style="width:150px">
			<select class="select" id="typeId" name="typeId">
				<option value="">--请选择类型--</option>
				<c:forEach items="${types}" var="type">
				<option value="${type.id}" ${params.typeId==type.id?'selected':''}>${type.name}</option>
				</c:forEach>
			</select>
		</span>
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>food/toEdit.htm?id="+id
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>food/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
...
//可适用产品
function canUseProducts(id){
    var title = "<i class='Hui-iconfont'>&#xe627;</i>可适用产品";
    var index = layer.open({
        type: 2,
        title: title,
        area: ['1000px', '500px'],
        content: "<%=basePath%>food/canUseProducts.htm?id="+id
    });
    layer.full(index);
}
</script>
