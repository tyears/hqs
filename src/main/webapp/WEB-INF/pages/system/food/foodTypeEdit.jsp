...
<article class="page-container">
	...
	<input type="hidden" name="modifyTime" value="<fmt:formatDate value="${info.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>类别名称：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" autocomplete="off" placeholder="类别名称" id="name" name="name" value="${info.name}"/>
		</div>
	</div>
	</form>
</article>
	...
	$(function(){
		...
		$("#myForm").validate({
			...
			submitHandler:function(form){
				...
				$.ajax({
					type:"post",
					url:"<%=basePath%>foodType/editSave.htm",
					dataType:"json",
					async:true,
					data:$(form).serialize(),
