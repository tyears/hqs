...
<div class="page-container">
	<form action="${className?uncap_first}/listPageData.htm" method="post" id="queryForm">
	<div class="text-c">
		...
		<#if column.isQuery=='y'>
		${column.remark}：<input type="text" id="${column.filedName}" name="${column.filedName}" value="${r'${params.'+column.filedName+'}'}" placeholder="${column.remark}" style="width:150px" class="input-text"/>
		</#if>
		</#list>
...
function edit(id){
	...
	var index = layer.open({
		...
		content: "<%=basePath%>${className?uncap_first}/toEdit.htm?id="+id
	});
	//layer.full(index);
}
...
function deleteObjs(id){
	var layerIndex = layer.confirm('确认要删除吗？',{icon:3},function(index){
		$.ajax({
			type:"post",
			url:"<%=basePath%>${className?uncap_first}/deleteData.htm",
			dataType:"json",
			async:false,
			data:{"id":id},
